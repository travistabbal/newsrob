...
public class ShowArticleActivity extends Activity implements IEntryModelUpdateListener, View.OnClickListener {
    ...
    private void createWebView(View parent) {
         ...
         * try {
         * 
         * Method m = wv.getClass().getMethod("setScrollbarFadingEnabled",
         * Boolean.class); m.invoke(wv, true); //
         * webView.setScrollbarFadingEnabled(true); // no longer the default //
         * for // Froyo } catch (Exception e) { // never mind }
         */
    ...
    private void newPosition() {
        ...
        try {
            ...
            PL.log("ADV: newPosition() Article " + selectedEntry.getTitle() + " was already read="
                    + articleWasAlreadyRead, this);
    ...
    private void viewAlternateContent() {
        ...
        if (isFullPageDownloaded()) {
            ...
            webView.loadDataWithBaseURL(selectedEntry.getBaseUrl(getEntryManager()), content,
                    "text/" + selectedEntry.getContentType(), "utf-8", null);
    ...
    protected void onPause() {
        ...
        PL.log("ADV: onPause() Article " + tmpTitle + " was already read=" + articleWasAlreadyRead
                + " leaving this activity=" + leavingThisActivity, this);
    ...
    protected void onResume() {
        ...
        if (currentTheme != getEntryManager().getCurrentThemeResourceId()) {
            ...
            Entry selectedEntry = getSelectedEntry();
            if (selectedEntry != null) {
                PL.log("ADV: onResume()1 Article " + selectedEntry.getTitle() + " was already read="
                        + articleWasAlreadyRead, this);

                // articleWasAlreadyRead = selectedEntry.getReadState() ==
                // ReadState.READ;
                PL.log("ADV: onResume()2 Article " + selectedEntry.getTitle() + " was already read="
                        + articleWasAlreadyRead, this);
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (!getEntryManager().areHoveringZoomControlsEnabled()) {
            menu.add(0, MENU_ITEM_ID_ZOOM_IN, 10, R.string.zoom_in).setIcon(R.drawable.gen_zoom_in)
                    .setAlphabeticShortcut('i');
            menu.add(0, MENU_ITEM_ID_ZOOM_OUT, 10, R.string.zoom_out).setIcon(R.drawable.gen_zoom_out)
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
        if (isContextMenuLaunchedOnLink()) {
            ...
            menu.add(0, ArticleViewHelper.MENU_ITEM_SHOW_IN_BROWSER_ID, 0, R.string.menu_show_in_browser)
                    .setTitleCondensed(U.t(this, R.string.menu_show_in_browser_condensed))
            ...
            menu.add(0, ArticleViewHelper.MENU_ITEM_SHARE_LINK_ID, 0, R.string.menu_item_share_link)
                    .setIntent(Intent.createChooser(sendIntent, null)).setIcon(android.R.drawable.ic_menu_share);
