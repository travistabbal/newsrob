...
import com.grazerss.widget.SwipeRelativeLayout;
  ...
  static void createArticleMenu(final Menu menu, final Activity owningActivity, final Entry selectedEntry)
  {
    ...
    if (false)
      menu.add(0, MENU_ITEM_SHARE_ARTICLE_ID, 7, "Share Article")
          ...
          .setEnabled(false && alternateHRefAvailable && selectedEntry.getContent() != null && selectedEntry.getContent().length() > 100);
      ...
      final Entry selectedEntry)
  {
    ...
    switch (item.getItemId())
    {
      ...
      case ArticleViewHelper.MENU_ITEM_MANAGE_FEED_ID:
        owningActivity.startActivity(new Intent(entryManager.getContext(), ManageFeedActivity.class).putExtra(ManageFeedActivity.EXTRA_FEED_ID,
            selectedEntry.getFeedId()));
        return true;
      case ArticleViewHelper.MENU_ITEM_REFRESH_CONTENT_ID:
        Toast.makeText(owningActivity, "The article's content is being removed. GrazeRSS will try to re-download it during the next sync.",
