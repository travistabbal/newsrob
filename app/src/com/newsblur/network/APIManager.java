...
public class APIManager {
    ...
    public APIManager(final Context context) {
        ...
        this.gson = new GsonBuilder().registerTypeAdapter(Date.class, new DateStringTypeAdapter())
    ...
    public StoriesResponse getStoriesForFeed(String feedId, String pageNumber, StoryOrder order, ReadFilter filter) {
        ...
        if (response.responseCode == HttpStatus.SC_OK && !response.hasRedirected) {
            ...
            //
            // Uri classifierUri =
            // FeedProvider.CLASSIFIER_URI.buildUpon().appendPath(feedId).build();
            //
            // contentResolver.delete(classifierUri, null, null);
            //
            // for (ContentValues classifierValues :
            // storiesResponse.classifiers.getContentValues()) {
    ...
    public StoriesResponse getStoriesForFeeds(String[] feedIds, String pageNumber, StoryOrder order, ReadFilter filter) {
        ...
        if (response.responseCode == HttpStatus.SC_OK && !response.hasRedirected) {
            ...
            // for (Story story : storiesResponse.stories) {
            // Uri storyUri =
            // FeedProvider.FEED_STORIES_URI.buildUpon().appendPath(story.feedId).build();
            // contentResolver.insert(storyUri, story.getValues());
            // insertComments(story);
            // }
            //
    ...
    public StoriesResponse getStarredStories(String pageNumber) {
        ...
        if (response.responseCode == HttpStatus.SC_OK && !response.hasRedirected) {
            ...
            // }
            // for (Story story : storiesResponse.stories) {
            // contentResolver.insert(FeedProvider.STARRED_STORIES_URI,
            // story.getValues());
            // insertComments(story);
            // }
            // for (UserProfile user : storiesResponse.users) {
    ...
    public SocialFeedResponse getSharedStoriesForFeeds(String[] feedIds, String pageNumber) {
        ...
        if (response.responseCode == HttpStatus.SC_OK && !response.hasRedirected) {
            ...
            // shared feeds (the first page), delete all previous shared feeds
            // if (TextUtils.equals(pageNumber,"1")) {
            // Uri storyUri = FeedProvider.ALL_STORIES_URI;
            // contentResolver.delete(storyUri, null, null);
            // }
    ...
    public SocialFeedResponse getStoriesForSocialFeed(String userId, String username, String pageNumber) {
        ...
        Uri feedUri = Uri.parse(APIConstants.URL_SOCIALFEED_STORIES).buildUpon().appendPath(userId)
     ...
     * 
     * @param doUpdateCounts
     *            forces a refresh of unread counts. This has a high latency
     *            cost and should not be set if the call is being used to
     *            display the UI for the first time, in which case it is more
     *            appropriate to make a separate, additional call to
     *            refreshFeedCounts().
     */
    public FeedFolderResponse getFolderFeedMapping(boolean doUpdateCounts) {
        ...
        if (response.responseCode == HttpStatus.SC_OK && !response.hasRedirected) {
            ...
            // prompt for login. We test this
            // // here, since this the first sync call we make on launch if we
            // believe we are cookied.
            ...
            // contentResolver.bulkInsert(FeedProvider.FEEDS_URI,
            // feedValues.toArray(new ContentValues[feedValues.size()]));
            // }
            //
            // for (String olderFeedId : existingFeeds.keySet()) {
            // if (feedUpdate.feeds.get(olderFeedId) == null) {
            // Uri feedUri =
            // FeedProvider.FEEDS_URI.buildUpon().appendPath(olderFeedId).build();
            // contentResolver.delete(feedUri, null, null);
            // }
            // }
            //
            // List<ContentValues> socialFeedValues = new
            ...
            // contentResolver.bulkInsert(FeedProvider.SOCIAL_FEEDS_URI,
            // socialFeedValues.toArray(new
            // ContentValues[socialFeedValues.size()]));
            // }
            //
            // Cursor folderCursor =
            // contentResolver.query(FeedProvider.FOLDERS_URI, null, null, null,
            // null);
            ...
            // folderCursor.close();
            //
            // for (final Entry<String, List<Long>> entry :
            // feedUpdate.folders.entrySet()) {
            ...
            // values.put(DatabaseConstants.FEED_FOLDER_FOLDER_NAME,
            // folderName);
            ...
            // ContentValues values = new ContentValues();
            // values.put(DatabaseConstants.STARRED_STORY_COUNT_COUNT,
            // starredStoriesCount);
    ...
    // Cursor feedCursor = contentResolver.query(FeedProvider.FEEDS_URI, null,
    // null, null, null);
    // feedCursor.moveToFirst();
    // HashMap<String, Feed> existingFeeds = new HashMap<String, Feed>();
    // while (!feedCursor.isAfterLast()) {
    // existingFeeds.put(Feed.fromCursor(feedCursor).feedId,
    // Feed.fromCursor(feedCursor));
    ...
    public void refreshFeedCounts() {
        ...
        if (response.responseCode == HttpStatus.SC_OK && !response.hasRedirected) {
            ...
            // FeedRefreshResponse.class);
            // for (String feedId : feedCountUpdate.feedCounts.keySet()) {
            // Uri feedUri =
            // FeedProvider.FEEDS_URI.buildUpon().appendPath(feedId).build();
            // contentResolver.update(feedUri,
            // feedCountUpdate.feedCounts.get(feedId).getValues(), null, null);
            // }
            //
            // for (String socialfeedId :
            // feedCountUpdate.socialfeedCounts.keySet()) {
            // String userId = socialfeedId.split(":")[1];
            // Uri feedUri =
            // FeedProvider.SOCIAL_FEEDS_URI.buildUpon().appendPath(userId).build();
            // contentResolver.update(feedUri,
            // feedCountUpdate.socialfeedCounts.get(socialfeedId).getValues(),
            // null,
            // null);
            // }
        }
